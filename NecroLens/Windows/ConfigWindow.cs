...
namespace NecroLens.Windows
{
    internal class ConfigWindow : Window
    {
        ...
        private void DrawEspTab()
        {
            ...
            var normalAggroColor = ImGui.ColorConvertU32ToFloat4(conf.NormalAggroColor);
            if (ImGui.ColorEdit4(Strings.ConfigWindow_ESPTab_ShowAggroRange_Proximity_and_Sight, ref normalAggroColor,
                                 ImGuiColorEditFlags.NoInputs))
            {
                conf.NormalAggroColor = ImGui.ColorConvertFloat4ToU32(normalAggroColor);
                conf.Save();
            }

            ImGui.SameLine();
            var soundAggroColor = ImGui.ColorConvertU32ToFloat4(conf.SoundAggroColor);
            if (ImGui.ColorEdit4(Strings.ConfigWindow_ESPTab_ShowAggroRange_Sound, ref soundAggroColor,
                                 ImGuiColorEditFlags.NoInputs))
            {
                conf.SoundAggroColor = ImGui.ColorConvertFloat4ToU32(soundAggroColor);
                conf.Save();
            }
